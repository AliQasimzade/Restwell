export const userSelect = state => state.auth.user;

export const languageSelect = state => state.application.language;

export const userInfo = state => state.user.userInfo;

export const wish = state => state.wish.wishlist

export const designSelect = state => state.config.design;

export const wishlistSelect = state => state.wishlist;

export const homeSelect = state => state.home;

export const listSelect = state => state.list;

export const searchSelect = state => state.search;

export const settingSelect = state => state.config.setting;

export const messengerSelect = state => state.messenger;

export const notificationSelect = state => state.notification;
